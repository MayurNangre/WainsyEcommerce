...
@section('content')
    <section class="mb-4 pt-3">
        <div class="container ">
            <div class="bg-white shadow-sm p-3 myrounded-20">
                <div class="row">
                                    ...
                                    <div class="mt-3">
                                        ...
                                        @else
                                            <div>
                                                ...
                                                <button type="button" class="btn btn-lg btn-primary w-100 buy-now fw-600 top20btn" style="display:flex; align-items:center; justify-content:center; margin-right: 20px" onclick="buyNow()">
                                                    <i class="la la-shopping-cart" style="font-size: 25px;"></i> {{ translate('Buy Now idhow ') }}
                                                </button>
                                                
                                            </div>
                                        @endif
                                        <button type="button" class="btn btn-lg btn-secondary out-of-stock fw-600 d-none" disabled>
                                            <i class="la la-cart-arrow-down top20btn"></i> {{ translate('Out of Stock') }}
                                        </button>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="text-left">
                            ...
                            <div class="row align-items-center">
                                <div class="col-6">
                                    ...
                                    @endphp
                                    <span class="rating opacity-80">
                                        {{ renderStarRating($detailedProduct->rating) }}
                                    </span>
                                    <span class="ml-1 opacity-80">({{ $total }} {{ translate('reviews') }})</span>
                                </div>
                            ...
                            <div class="row align-items-center">
                                <div class="col-auto">
                                    <small class="mr-2 opacity-80">{{ translate('Sold by') }}: </small><br>
                                    @if ($detailedProduct->added_by == 'seller' && get_setting('vendor_system_activation') == 1)
                                        <a href="{{ route('shop.visit', $detailedProduct->user->shop->slug) }}" class="text-reset ">{{ $detailedProduct->user->shop->name }}</a>
                                    @else
                                        <span class="font-weight-bold opacity-80">{{ translate('Inhouse product') }}</span>
                                    @endif
                                </div>
                                @if (get_setting('conversation_system') == 1)
                                    <div class="col-auto">
                                        <button class="btn btn-sm btn-soft-primary top20btn" onclick="show_chat_modal()">{{ translate('Message Seller') }}</button>
                                    </div>
                                @endif
                            ...
                            @else
                                @if (home_price($detailedProduct) != home_discounted_price($detailedProduct))
                                    ...
                                    <div class="row no-gutters my-2">
                                        <div class="col-sm-2">
                                            <div class="opacity-50 text-dark font-weight-bold">{{ translate('Discount Price') }}:</div>
                                        </div>
                                        <div class="col-sm-10">
                                            <div class="">
                                ...
                                @else
                                    <div class="row no-gutters mt-3">
                                        <div class="col-sm-2">
                                            <div class="opacity-80 text-dark font-weight-bold my-2">{{ translate('Price') }}:</div>
                                        </div>
                                        <div class="col-sm-10">
                                            <div class="">
                                                <strong class="h2 fw-600 text-primary">
                            ...
                            <form id="option-choice-form">
                                ...
                                <div class="row no-gutters">
                                    <div class="col-sm-2">
                                        <div class="opacity-80 text-dark font-weight-bold my-2">{{ translate('Quantity') }}:</div>
                                    </div>
                                    <div class="col-sm-10">
                                        <div class="product-quantity d-flex align-items-center">
                                            ...
                                            @endphp
                                            <div class="avialable-amount opacity-80">
                                ...
                                <div class="row no-gutters pb-3 d-none" id="chosen_price_div">
                                    <div class="col-sm-2">
                                        <div class="opacity-80 font-weight-bold my-2">{{ translate('Total Price') }}:</div>
                                    </div>
                                    <div class="col-sm-10">
                            ...
                            <div class="mt-3">
                                @if ($detailedProduct->external_link != null)
                                    <a type="button" class="btn btn-primary buy-now fw-600 top20btn" href="{{ $detailedProduct->external_link }}">
                                        <i class="la la-share"></i> {{ translate($detailedProduct->external_link_btn) }}
                                    </a>
                                @else

                                    <button type="button" class="btn btn-soft-primary mr-2 add-to-cart fw-600 top20btn" onclick="addToCart()">
                                        <i class="las la-shopping-bag" style="font-size: 20px"></i>
                                        <span class="d-none d-md-inline-block"> {{ translate('Add to cart') }}</span>
                                    </button>

                                    <button type="button" class="btn btn-soft-primary mr-2 Add to wishlist fw-600 top20btn" onclick="addToWishList({{ $detailedProduct->id }})">
                                        <i class="la la-shopping-cart" style="font-size: 20px"></i>
                                        <span class="d-none d-md-inline-block"> {{ translate('Add to wishlist') }}</span>
                                    </button>

                                    <button type="button" class="btn btn-soft-primary mr-2 Add to compare fw-600 top20btn" onclick="addToCompare({{ $detailedProduct->id }})">
                                        <i class="la la-shopping-cart" style="font-size: 20px"></i>
                                        <span class="d-none d-md-inline-block"> {{ translate('Add to compare') }}</span>
                                    </button>
                                    
                                    {{-- <button type="button" class="btn btn-soft-primary Add to compare fw-600" onclick="addToCompare({{ $detailedProduct->id }})">
                                        <i class="la la-shopping-cart" style="font-size: 20px"></i> {{ translate('Add to compare')}}
                                    </button> --}}
                                @endif
                                <button type="button" class="btn btn-secondary out-of-stock fw-600 d-none" disabled>
                                    <i class="la la-cart-arrow-down" style="font-size: 20px"></i> {{ translate('Out of Stock') }}
                                </button>
                            </div>
