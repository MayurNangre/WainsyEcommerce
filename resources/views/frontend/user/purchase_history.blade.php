                                ...
                                <tr>
									...
									@endif--}}
										@if($orderStatus == "cancelled" || $waybill == "")
											<a href="javascript:void(0)" class="btn btn-soft-danger btn-icon btn-circle btn-sm confirm-delete @if($order->pickup_created==1) disabled @endif" data-href="{{route('orders.destroy', $order->id)}}" title="{{ translate('Delete') }}">
                                               <i class="las la-trash"></i>
                                           </a>
										@else
											<a href="javascript:void(0)" class="btn btn-soft-danger btn-icon btn-circle btn-sm @if($order->pickup_created==1) disabled @endif" onclick="show_cancel_order({{ $order->id }})" title="{{ translate('Cancel Order') }}">
											<span>X</span>
											</a>
										@endif
