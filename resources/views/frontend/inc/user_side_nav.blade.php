            ...
            <ul class="aiz-side-nav-list px-2" data-toggle="aiz-side-menu">
                ...
                @else
                    ...
                    @if(Auth::user()->user_type == 'seller')
                        ...
                        @endif
                        @php
                                    $order = DB::table('orders')->where('seller_id',Auth::user()->id)->where('pickup_created', 0)->count();
                        @endphp
                        <li class="aiz-side-nav-item">
                            <a href="{{route('pickup_dashboard')}}" class="aiz-side-nav-link {{ areActiveRoutes(['pickup_dashboard'])}}">
                                <i class="las la-money-bill aiz-side-nav-icon"></i>
                                <span class="aiz-side-nav-text">{{ translate('Pickup Request') }}</span>
                                @if($order > 0)<span class="badge badge-info">{{ $order }}</span> @endif
                            </a>
                        </li>
                        <li class="aiz-side-nav-item">
