 ...
 */
            ...
            $order_id = DB::table('order_details')->where('order_id', $order->id)->get();
            $temp_product = Product::find($order_id[0]->product_id);

            $product_isDigital = Category::find($temp_product->category_id);
            // dd($product_isDigital);


            if ($temp_product->added_by == "admin") {
                $data['pickup_name']     = "SF SIRAOM 0121083";
            } else {
                $seller = DB::table('sellers')->where('user_id', $order_id[0]->seller_id)->get();
                $data['pickup_name'] = $seller[0]->warehouse_name;
            }
            ...
            if ($product_isDigital->digital == 0) {
    ...
    public function cancel()
    {
        ...
        foreach ($combined_order->orders as $order) {
            ...
            $product_stock = ProductStock::where('product_id', $order->product_id)->where('variant', $order->variation)->first();
